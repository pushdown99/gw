...
static void* mqtt_task(void* args) {
    ...
    rc = MQTTAsync_createWithOptions(&client, "127.0.0.1:1883", clientid, MQTTCLIENT_PERSISTENCE_NONE, NULL, &create_opts);
...
static void kaa_logging_serialize(avro_writer_t writer, void *data) {
    if (data) {
        kaa_logging_t *record = (kaa_logging_t *)data;
        kaa_int_serialize(writer, &record->temperature);
    }
}

static size_t kaa_logging_get_size(void *data) {
    if (data) {
...
kaa_logging_t *kaa_logging_create(void) {
    kaa_logging_t *record = (kaa_logging_t *)KAA_CALLOC(1, sizeof(kaa_logging_t));

    if (record) {
        record->serialize = kaa_logging_serialize;
        record->get_size  = kaa_logging_get_size;
        record->destroy   = kaa_data_destroy;
    }
    return record;
}

kaa_logging_t *kaa_logging_deserialize(avro_reader_t reader) {
    kaa_logging_t *record = (kaa_logging_t *)KAA_MALLOC(sizeof(kaa_logging_t));
...
static void example_callback(void *context)
{
    ...
    if (difftime(current_time, last_sample_time) >= sample_period) {
        ...
        // hyhwang
        if(temperature > 0) {
        printf("(hyhwang) Logging record create with temperature %d \n", temperature);
        kaa_logging_t *log_record = kaa_logging_create();
...
int main(void) {
    char buf[BUFSIZ];
    getconf("/opt/radix/bin/db.properties", "SDK_TOKEN", buf);
    printf("TOKEN: %s\n", buf);
    ...
    const kaa_root_configuration_t *dflt = kaa_configuration_manager_get_configuration(kaa_client_get_context(kaa_client)->configuration_manager);
